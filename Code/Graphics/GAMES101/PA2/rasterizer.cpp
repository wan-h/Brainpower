...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    // 找到最小外接矩形
    int x1 = std::min({t.v[0].x(), t.v[1].x(), t.v[2].x()});
    int x2 = std::max({t.v[0].x(), t.v[1].x(), t.v[2].x()});
    int y1 = std::min({t.v[0].y(), t.v[1].y(), t.v[2].y()});
    int y2 = std::max({t.v[0].y(), t.v[1].y(), t.v[2].y()});
    // 遍历最小外接矩形找到在三角形里面的点
    for (int i = x1; i < x2; i++) {
        for (int j = y1; j < y2; j++) {
            // 在三角形内，这里看到是中心点，所以加0.5
