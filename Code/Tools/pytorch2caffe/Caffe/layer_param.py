...
class Layer_param():
    ...
    def flatten_param(self, start_dim=1, end_dim=-1):
        flatten_param = pb.FlattenParameter()
        flatten_param.axis = start_dim
        if end_dim != -1:
            flatten_param.end_axis = end_dim
    ...
    def interp_param(self, size):
        interp_param = pb.InterpParameter()

        if size:
            if isinstance(size, int):
                interp_param.height = size
                interp_param.width = size
            else:
